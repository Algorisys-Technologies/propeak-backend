...
exports.getVisitingCardsAccountWise = (req, res) => {
  ...
  let pathName =
    req.body.pathData === "root" ? "/" : req.body.pathData.toLowerCase();
  let itemArray = [];
  ...
  UploadRepositoryFile.find({
    ...
    .then(async (result) => {
      ...
      for (let j = 0; j < itemArray.length; j++) {
        ...
        let pathDt =
          pathName === "/" ? itemArray[j] : `${pathName}${itemArray[j]}`;
    ...
    .catch((error) => {
      ...
      res
        .status(500)
        .json({ error: "An error occurred while fetching contacts files." });
    });
};

exports.getVisitingCardsFolderWise = async (req, res) => {
    try {
      const companyId = req.body.companyId;
      ...
      let folder;
      if(vfolderId){
        folder = await vfolderModel.findOne({_id: vfolderId})
      }
      // Ensure the "contacts" folder exists for the given company
      if (!fs.existsSync(companyContactsFolder)) {
        fs.mkdirSync(companyContactsFolder, { recursive: true });
      ...
      if (page === "All") {
        ...
        result = await UploadRepositoryFile.find(query).populate("contactId", "title");
      } else {
        ...
        totalPages = Math.ceil((await UploadRepositoryFile.countDocuments(query)) / limit);
      }
  
      let folderPath = req.body.pathData === "root" ? companyContactsFolder : `${companyContactsFolder}${pathName}`;
...
exports.getAllRepositoryFile = (req, res) => {
  ...
  if (!fs.existsSync(companyDocumentFolder)) {
  ...
  UploadRepositoryFile.find({
    ...
    .then(async (result) => {
      ...
      let folderPath =
        req.body.pathData === "root"
          ? uploadFolder
          : uploadFolder + "/" + companyId + "/documents" + pathName;
...
exports.postMultipleVisitingCards = async (req, res) => {
  try {
    ...
    let vFolder = await VFolder.findOne({
      name: req.body.folderName,
      companyId,
    });

    if (!vFolder) {
      vFolder = await VFolder.create({
        name: req.body.folderName,
        isDeleted: false,
        companyId,
        created_on: new Date(),
    ...
    const uploadedVisitingCards = await UploadRepositoryFile.insertMany(
      uploadFiles
    );
...
exports.editRepositoryFile = (req, res) => {
  ...
  UploadRepositoryFile.findOneAndUpdate(
    { _id: req.body._id },
    updatedFile
  ).then((result) => {
    console.log("result", result);
